	...
	public class PointCloud2BinaryConverter : EditorWindow {
        ...
        */
		...
		void ConvertBinaryV1() {
			...
			if (!ValidateSaveAndRead(saveFilePath, fileToRead))
				return;
			...
			//using (BufferedStream streamReader = new BufferedStream(fs))
			{
				...
				while (haveMoreToRead == true) {
					...
					if (rowCount >= masterPointCount || streamReader.EndOfStream) {
						...
						if (skippedRows > 0)
							Debug.LogWarning("Parser skipped " + skippedRows + " rows (probably bad data or comment lines in point cloud file)");
		...
		void LASConverter() {
			...
			if (fileSignature != "LASF")
				Debug.LogError("LAS> FileSignature error: '" + fileSignature + "'");
			...
			*/
			...
			if (pointDataRecordFormat != 2 && pointDataRecordFormat != 3)
				Debug.LogWarning("LAS Import might fail - only pointDataRecordFormat #2 & #3 are supported (Your file is " + pointDataRecordFormat + ")");
			if (versionMinor != 2)
				Debug.LogWarning("LAS Import might fail - only version LAS 1.2 is supported. (Your file is " + versionMajor + "." + versionMinor + ")");

			masterPointCount = (int)legacyNumberOfPointRecords;

			// scaling enabled, scale manual offset
			if (useScaling == true)
				manualOffset *= scaleValue;
			...
			using (BinaryWriter writer = new BinaryWriter(File.Open(saveFilePath, FileMode.Create))) {
				...
				while (haveMoreToRead == true) {
					...
					if (pointDataRecordFormat == 3)
						reader.ReadBytes(8); // GPS Time for format#3
