Shader "UnityCoder/PointCloud/DX11/TextureCloud" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass
		{
			...
			StructuredBuffer<float> buf_Points;
			...
			GS_INPUT VS_Main(uint id : SV_VertexID, uint inst : SV_InstanceID)
			{
				...
				float unnormalizer = 10;
				
				float imageSize = 1000;

				float x = id % imageSize;
				float y = id / imageSize;
			    float4 tex = tex2Dlod(_Cloud, float4(x,y,0,0));
				
				float3 p = tex.xyz*unnormalizer;
				
				o.pos = UnityObjectToClipPos(p);
				fixed3 col = tex.rgb; //float3(buf_Points[id],0,0);
