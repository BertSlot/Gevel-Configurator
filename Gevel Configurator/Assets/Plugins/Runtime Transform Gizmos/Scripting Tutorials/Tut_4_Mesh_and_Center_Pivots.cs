...
namespace RTG {
	...
	public class Tut_4_Mesh_and_Center_Pivots : MonoBehaviour {
		...
		/// </summary>
		private Dictionary<GameObject, Transform> _clipboard = new Dictionary<GameObject, Transform>();
		...
		private void Update() {
			...
			if (Input.GetMouseButtonDown(0) &&
				...
				if (pickedObject != null) {
					...
					if (pickedObject.GetComponent<Outline>() == null) {
						pickedObject.AddComponent<Outline>();

					}

					Outline outline = pickedObject.GetComponent<Outline>();
		...
		private List<GameObject> Paste() {
			...
			foreach (KeyValuePair<GameObject, Transform> obj in _clipboard) {
				instantiatedObjects.Add(Instantiate(obj.Key, obj.Value.position, obj.Value.rotation) as GameObject);
			}
		...
		private void CopyToClipboard(List<GameObject> gameObjects) {
			...
			foreach (var obj in gameObjects) {
				...
				_clipboard.Add(obj, objTransform);
			}
		}
